...
- **`scroll` reports actual delta.** Before: always reported success, misleading when a fixed-viewport page refused to scroll or was already at the edge. Now: captures `window.scrollY` before/after, reports `(no-op — page not scrollable …)` when zero delta, or `(actual: Npx — reached document edge)` when partial. Uses `behavior: "instant"` so sites with CSS `scroll-behavior: smooth` don't make the scroll async (without that, the after-value equals the before-value and every scroll would be mis-reported as a no-op).
